...
export const renderTasks = (listId, tasksArray) => {
  ...
  tasksArray.forEach((task) => {
    ...
    // label.append(task.name); //DELETE
    const textBody = taskTemplate.querySelector('[data-task-text]')
    textBody.innerHTML = task.name;
...
export const renderPage = (items) => {
  ...
  items.forEach((col) => {
    ...
    spanDollar.dataset.colDollar = col.columnId
    columnHeader.appendChild(spanDollar)
...
export const displayMoneyTotal = (sum0, sum1, sum2) => {
  document.querySelector('[data-col-dollar="col-0"]').innerText = ` $${sum0}`,
    document.querySelector('[data-col-dollar="col-1"]').innerText = ` $${sum1}`,
    document.querySelector('[data-col-dollar="col-2"]').innerText = ` $${sum2}`
