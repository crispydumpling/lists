...
export const getDollarTotal = () => {
  let col = [0, 0, 0]
  for (let i = 0; i < todoData.length; i++) {
    // console.log(todoData[i].lists)
    let total = [];
    todoData[i].lists.forEach(list => {
      // console.log(list.tasks)
      list.tasks.forEach(task => {
        if (task.tag != '') {
          let val = parseInt(task.tag.slice(1))
          total.push(val)
        }
      })
    })
    col[i] = total.reduce(((accumulator, currentValue) => accumulator + currentValue), 0)
  }
  // console.log(col)
  return col;
}